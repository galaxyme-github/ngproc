...
//$product_listing_t	 = $slug;
	...
	function implecode_save_products_meta( $post_id, $post, $post_prev = null ) {
		remove_filter( 'get_post_metadata', array( $this, 'hidden_product_data' ), 10, 3 );
		...
		if ( $post_type_now == 'al_product' ) {
			...
			if ( in_array( $post->post_status, ic_visible_product_status() ) ) {
				...
				foreach ( $product_meta as $key => $value ) {
					if ( in_array( $key, $current_post_keys ) ) {
	...
	function set_product_messages( $messages ) {
		...
		if ( $quasi_post_type == 'al_product' ) {
			...
			$messages[ $post_type ] = array(
				0	 => '',
				1	 => sprintf( __( '%1$s updated. <a href="%2$s">View %1$s</a>', 'ecommerce-product-catalog' ), ic_strtolower( $singular ), esc_url( get_permalink( $post_ID ) ) ),
