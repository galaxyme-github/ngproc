								...
								<script>
									jQuery( document ).ready( function () {
										jQuery( "input[name=ic-fake-empty-feature]" ).click( function () {
											alert( "<?php echo sprintf( __( 'You have to remove the %s from the main catalog page to switch the integration method.', 'ecommerce-product-catalog' ), ic_catalog_shortcode_name() ) ?>" );
...
function get_product_slug() {
	...
	if ( empty( $slug ) ) {
		$settings	 = get_multiple_settings();
		$slug		 = sanitize_title( $settings[ 'catalog_plural' ] );
	}
