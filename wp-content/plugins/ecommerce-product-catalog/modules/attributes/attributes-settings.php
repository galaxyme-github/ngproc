				...
				<form method="post" action="options.php">
					...
					if ( $attributes_count > 0 ) {
						...
						<div class="al-box info">
							<p><?php _e( "If you fill out the fields below, the system will automatically pre-fill the fields on product pages, so you don't have to fill them every time you add product.", 'ecommerce-product-catalog' ) ?></p><p><?php _e( "When every product in your catalogue is different, you can leave all or a part of these fields empty.", 'ecommerce-product-catalog' ) ?></p><p><?php _e( 'You can change these default values for every item.', 'ecommerce-product-catalog' ); ?></p>
						</div>
...
function ic_listing_attributes_settings( $listing_settings, $listing_name ) {
	...
	?>
	<input title="<?php _e( 'Use this only with short attributes labels and values, e.g. Color: Red', 'ecommerce-product-catalog' ) ?>" type="checkbox" name="<?php echo $option_name ?>" value="1"<?php checked( 1, isset( $listing_settings[ 'attributes' ] ) ? $listing_settings[ 'attributes' ] : ''  ); ?>> <?php _e( 'Show Attributes', 'ecommerce-product-catalog' ); ?><br><?php
}
...
function ic_available_size_units() {
	$units = array( 'disable' => __( 'Disable Size', 'ecommerce-product-catalog' ), 'm' => 'm', 'cm' => 'cm', 'mm' => 'mm', 'in' => 'in', 'yd' => 'yd', 'ft' => 'ft' );
	return $units;
}
...
function ic_product_attribute_label_update( $new_value, $old_value ) {
	...
	foreach ( $old_value as $i => $old ) {
		if ( in_array( $old, $new_value ) || (!empty( $new_value[ $i ] ) && $old === $new_value[ $i ]) ) {
			continue;
		} else {
			$new = strval( $new_value[ $i ] );
		}
		if ( empty( $new ) || (!empty( $new ) && in_array( $new, $all_labels )) ) {
			continue;
		}
		...
		wp_update_term( $attribute_id, 'al_product-attributes', array( 'parent' => 0, 'name' => $new, 'slug' => $new ) );
	}
