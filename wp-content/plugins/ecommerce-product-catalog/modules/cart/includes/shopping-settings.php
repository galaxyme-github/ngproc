...
function shopping_cart_menu() {
	?>
	<a id="shopping-cart-settings" class="element" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=shopping-cart' ) ?>"><?php echo apply_filters( 'ic_simple_cart_settings_menu_name', __( 'Shopping Cart', 'ecommerce-product-catalog' ) ) ?> </a><?php
}
...
function get_shopping_cart_settings() {
	$shopping_cart_settings = ic_get_global( 'shopping_cart_settings' );
	if ( $shopping_cart_settings ) {
	...
	ic_save_global( 'shopping_cart_settings', $shopping_cart_settings );
	return apply_filters( 'ic_shopping_cart_settings', $shopping_cart_settings );
}
	...
	if ( $submenu == 'shopping-cart' ) {
		...
		<div class="shopping-cart-settings setting-content submenu">
			<form method="post" action="options.php">
				...
				<table>
					<tr>
						...
						<td>
							...
							ic_select_page( 'shopping_cart_settings[shopping_cart_page]', __( 'Select Cart Page', 'ecommerce-product-catalog' ), $shopping_cart_settings[ 'shopping_cart_page' ], true, false, 1, false, '', array( 'title' => __( 'Cart', 'ecommerce-product-catalog' ), 'content' => $default_content, 'option' => 'shopping_cart_settings', 'option_sub' => 'shopping_cart_page' ) );
							?>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							ic_select_page( 'shopping_cart_settings[cart_submit_page]', __( 'Select Submit Page', 'ecommerce-product-catalog' ), $shopping_cart_settings[ 'cart_submit_page' ], true, false, 1, false, '', array( 'title' => __( 'Checkout', 'ecommerce-product-catalog' ), 'content' => $default_content, 'option' => 'shopping_cart_settings', 'option_sub' => 'cart_submit_page' ) );
							?>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							ic_select_page( 'shopping_cart_settings[thank_you_page]', __( 'Select Thank You Page', 'ecommerce-product-catalog' ), $shopping_cart_settings[ 'thank_you_page' ], true, false, 1, false, '', array( 'title' => __( 'Thank You', 'ecommerce-product-catalog' ), 'content' => $default_content, 'option' => 'shopping_cart_settings', 'option_sub' => 'thank_you_page' ) );
							?>
						</td>
					</tr>
				</table><?php implecode_al_box( __( 'Remember to use the "Product Cart" widget to show the cart in the sidebar.', 'ecommerce-product-catalog' ) ); ?>
				...
				<?php
				implecode_info( sprintf( __( '%1$sSMTP plugin%2$s is recommended to make sure that your notifications are not treated as SPAM.', 'ecommerce-product-catalog' ), '<a target="_blank" href="' . admin_url( 'plugin-install.php?s=smtp&tab=search&type=term' ) . '">', '</a>' ) );
				?>
				<table>
					<?php
				...
				<table><?php
					implecode_settings_radio( __( 'Remember fields by', 'ecommerce-product-catalog' ), 'shopping_cart_settings[form_registration]', $shopping_cart_settings[ 'form_registration' ], array( 'user' => __( 'User Account', 'ecommerce-product-catalog' ), 'cookie' => __( 'Cookie Save', 'ecommerce-product-catalog' ) ) );
					...
					implecode_settings_checkbox( __( 'Disable State Field', 'ecommerce-product-catalog' ), 'shopping_cart_settings[disable_state]', $shopping_cart_settings[ 'disable_state' ] );
					?>
				</table>
				<h3><?php _e( 'Orders', 'ecommerce-product-catalog' ); ?></h3>
				<table>
					<?php
					implecode_settings_checkbox( __( 'Disable Order Registration', 'ecommerce-product-catalog' ), 'shopping_cart_settings[order_registration_disable]', $shopping_cart_settings[ 'order_registration_disable' ] );
					?>
				</table>
...
//$default = '{"fields":[{"label":"Name","field_type":"text","required":true,"field_options":{"size":"medium"},"cid":"name"},{"label":"Email","field_type":"email","required":true,"field_options":{"size":"medium"},"cid":"email"},{"label":"Subject","field_type":"text","required":true,"field_options":{"size":"medium"},"cid":"subject"},{"label":"Message","field_type":"paragraph","required":true,"field_options":{"size":"medium"},"cid":"message"}]}';
	...
	if ( empty( $shopping_cart_settings[ 'disable_state' ] ) ) {
		$default .= ',{"label":"' . __( 'State', 'ecommerce-product-catalog' ) . ':","field_type":"dropdown","required":false,"field_options":{"size":"medium"';
		$default .= ',"options":[';
		foreach ( $supported_states as $code => $state ) {
			$default .= '{"label":"' . $state . '", "checked":false},';
		}
