	...
	static function payment_details( $order ) {
		if ( true || isset( $_POST[ 'cart_submit' ] ) ) {
			...
			foreach ( $products_array as $cart_id => $p_quantity ) {
				...
				$product_price			 = ic_payment_number_format( $product_price );

				if ( $taxed ) {
					if ( !empty( $currency_settings[ 'tax_included' ] ) ) {
						$tax_rate_c		 = $tax_rate[ 'tax_rate' ] / 100;
						$product_price	 = ic_roundto( $product_price / (1 + $tax_rate_c), $tax_rate[ 'tax_rate_round' ] );
					}
					$product_tax		 = ic_get_price_tax( $product_price, $product_id );
					$order[ 'tax' ][]	 = $product_tax;
				}
				$order[ 'sum' ][]	 = $product_price;
				$product_total		 = $product_price * $p_quantity;
				if ( $taxed ) {
					$taxed_product						 = ic_roundto( $product_price + $product_tax, $tax_rate[ 'tax_rate_round' ] );
					$taxed_total						 = $taxed_product * $p_quantity;
					$order[ 'product_gross' ][]			 = $taxed_product;
					$order[ 'product_total_net' ][]		 = $product_total;
					$order[ 'product_total_gross' ][]	 = $taxed_total;
					$order[ 'product_total' ][]			 = $taxed_total;
...
class ic_cart_checkout_form_email {
	...
	function products_summary( $who = '' ) {
		...
		foreach ( $products_array as $cart_id => $p_quantity ) {
			...
			$product_total						 = $product_price * $p_quantity;
			ic_cart_update_tax( $product_id, $product_total, $cart_id );
