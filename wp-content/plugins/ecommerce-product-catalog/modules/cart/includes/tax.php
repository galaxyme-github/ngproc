...
function ic_get_checkout_field_selector( $fields, $pre_name, $name, $selected, $attr = 'multiple',
...
function ic_cart_update_tax( $product_id, $p_total, $cart_id ) {
...
function ic_cart_get_tax() {
	$current_cart_tax	 = ic_get_global( 'current_cart_tax' );
	$tax				 = 0;
	if ( !empty( $current_cart_tax ) ) {
		foreach ( $current_cart_tax as $tax_sum ) {
			$tax += $tax_sum;
		}
	}
	return $tax;
...
function ic_get_tax_rate( $product_id ) {
