...
class ic_cart_shipping {
	...
	function settings_html() {
		...
		echo '<table>';
		implecode_settings_radio( __( 'Shipping Cost Mode', 'ecommerce-product-catalog' ), 'general_shipping_settings[cart_shipping][mode]', $cart_shipping[ 'mode' ], array( 'highest' => __( 'Highest cost of all products in cart', 'ecommerce-product-catalog' ), 'individual' => __( 'Sum of each product shipping cost', 'ecommerce-product-catalog' ), 'none' => __( 'Shipping cost calculation disabled', 'ecommerce-product-catalog' ) ) );
		echo '</table>';
	}
			...
			foreach ( $shipping_labels as $cart_id => $ship ) {
				...
				foreach ( $ship[ 'labels' ] as $i => $name ) {
					if ( !empty( $name ) ) {
						...
						$shipping[ $cart_id ][ 'options' ][ $name ]	 = array( 'name' => $name, 'price' => $ship[ 'prices' ][ $i ] );
					}
				}
			}
		}
	...
	function add_current( $content = '', $pre_name = 'cart_' ) {
		...
		if ( is_array( $shipping_options ) && (count( $shipping_options ) > 1 || (is_array( $first_shipping_options[ 'options' ] ) && count( $first_shipping_options[ 'options' ] ) > 1)) ) {
			$a				 = 0;
	...
	function shipping_total_table_row( $products_table, $price, $cart ) {
		...
		if ( $price ) {
			...
			if ( is_array( $shipping_options ) && is_array( $first_option[ 'options' ] ) && (count( $shipping_options ) <= 1 && count( $first_option[ 'options' ] ) <= 1) ) {
