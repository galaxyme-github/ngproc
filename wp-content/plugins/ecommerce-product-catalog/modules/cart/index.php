...
if ( !class_exists( 'ic_shopping_ecommerce_cart' ) ) {
	...
	class ic_shopping_ecommerce_cart {
		...
		function register_styles() {
			wp_register_style( 'al_shopping_cart_styles', AL_SC_BASE_URL . 'css/shopping-cart.css?' . filemtime( AL_SC_BASE_PATH . '/css/shopping-cart.css' ), array( 'dashicons' ) );
			wp_register_script( 'al_shopping_cart_scripts', AL_SC_BASE_URL . 'js/shopping-cart.js?' . filemtime( AL_SC_BASE_PATH . '/js/shopping-cart.js' ) );
		}
		...
		function enqueue_styles() {
			...
			wp_localize_script( 'al_shopping_cart_scripts', 'ic_cart_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'dec_sep' => $currency[ 'dec_sep' ], 'th_sep' => $currency[ 'th_sep' ], 'total_label' => __( 'Purchase Total', 'ecommerce-product-catalog' ) ) );
		}
		...
		function enqueue_admin_styles() {
			wp_localize_script( 'ic_shopping_cart_admin', 'admin_cart_object', array( 'tax_limit_placeholder' => __( 'Leave empty for any', 'ecommerce-product-catalog' ), 'tax_limit_label' => sprintf( __( '%s values', 'ecommerce-product-catalog' ), '[field_name]' ) ) );
