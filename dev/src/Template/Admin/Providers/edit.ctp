 ...
 */
?>
<?php $this->assign('title', $provider->name);?>
...
<div class="dashboard-container">
    ...
    <div class="row partner-form" id="partner-form">
        ...
        <div class="form-row">
            ...
            <button type="button" id="btn-banir" class="btn btn-danger col-md-2 btn-edit2">Excluir</button>            
        </div>
    </div>
        <?= $this->Form->end() ?>
    </div>
...
$(document).ready(function() {
        $('#btn-banir').click(function() {
            swal({
                title: "Tem certeza?",
                text: "Confirme a exclusão do fornecedor <?= $provider->id ?>.",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
            .then((confirm) => {
                if (confirm) {
                    window.location.href = "<?=$this->Url->build(['controller' => 'Providers', 'action' => 'delete',  $provider->id])?>";
                    swal("fornecedor ecluido com sucesso", {
                        icon: "success",
                    });
                } else {
                    return;
