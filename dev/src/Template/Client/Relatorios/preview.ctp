                ...
                <nav class="nav_tabs">
                    <ul id="nav_tabs_ul">
                        ...
                        <?php $arrayProvidersCotation = []; ?>
                            ...
                            // }
                            ?>
                                <li id="aba-f<?=$cont?>"><input type="radio" name="tabs" class="rd_tabs" id="tab<?=$cont?>" <?= $cont == 0 ? 'checked' : '' ?>><label class="active" for="tab<?=$cont?>" onclick="trocarDeTela(<?=$cont?>,0)"><?= 'Aba '. ($cont + 1)//formatarNomeFornecedor($nomeFornecedor)?></label></li>
                                <?php $cont++?>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </ul>
                    ...
                    <div id="partner-view-product">
                        <div>
                            ...
                            <div class="content-right float-right col-sm-4">
                                ...
                                <?php if (empty($cotation->cotation_attachments)) echo '<p style="padding-left=15px">Não há</p>' ?>
                                <!-- <?php foreach ($cotation->cotation_attachments as $anexo) : ?>
                                    <div class="col-sm-12">
...
<div class="modal fade" id="modal_payment<?= $cotation->id ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <h5 class="modal-title" id="exampleModalLabel">Forma de Pagamento</h5>
            ...
            <h5 style="text-align: center;">
                <!-- <button type="button" class="btn btn-primary" onclick="window.location.href='<?= $this->Url->build(['action' => 'createPaymentPaypal', $cotation->id])?>'">Prosseguir</button> -->
                <!-- <button type="button" class="btn btn-primary" onclick="window.location.href='<?= $this->Url->build(['action' => 'createPaymentPagSeguro', '?' => ['id' => $cotation->id, 'valor' => $pay, 'mainId' => $main_cotation->id]])?>'">Prosseguir</button> -->
                <button type="button" class="btn btn-primary" onclick="enviaPagseguro(<?= $cotation->id ?> , <?= $pay ?>, <?=$main_cotation->id?>)">Prosseguir</button>
            </h5>
            <h5 style="text-align: center;">
